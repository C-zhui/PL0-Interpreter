	...
	public:
		...
		void declareConst() {
			...
			do {
				...
				if (id.type == Symbol::identifier) {
					...
					if (eql.type == Symbol::eql) {
						...
						if (num.type == Symbol::number) {
							...
							int constV;
							iss >> constV;
							if (!identifierTable.add(id.value, true, constV))
								throw string("repeatedly declare const : ") + id.value;
						}
				...
				id = ts.move().peek();
			} while (id.type == Symbol::comma);
			if (id.type != Symbol::semicolon)
